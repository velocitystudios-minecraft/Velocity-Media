package fr.velocity.video.util.displayers;
...
public class VideoDisplayer implements IDisplay {
    ...
    public static IDisplay createVideoDisplay(Vec3d pos, String url, float volume, float minDistance, float maxDistance, boolean loop, boolean playing) {
        return TryCore.withReturn((defaultVar) -> {
            ...
            TextureCache cache = TextureCache.get("https://i.imgur.com/XCcN2uX.png");
            return cache.ready() ? (url.equals("https://i.imgur.com/XCcN2uX.png") ? null : cache.createDisplay(pos, "https://i.imgur.com/XCcN2uX.png", volume, minDistance, maxDistance, loop, playing)) : null;
        }).get());
    }
